---
layout: default
title: 'Training Contents August 2025'
nav_order: 1
has_children: false
parent: 'Training Contents'
---

# Training Contents August 2025
{: .no_toc }

Training content for August 2025

## Table of contents
{: .no_toc .text-delta }

1. TOC
{:toc}

<br/>

---

<br/>

## Learning Path 1: Develop generative AI apps in Azure

- [Plan and prepare to develop AI solutions on Azure](https://aka.ms/mslearn-prepare-ai-dev)

- [Choose and deploy models from the model catalog](https://aka.ms/mslearn-ai-models)

- [Exercise: Choose and deploy a language model](https://microsoftlearning.github.io/mslearn-ai-studio/Instructions/02-Explore-model-catalog.html)

- [Develop an AI app with the Azure AI Foundry SDK](https://aka.ms/mslearn-ai-foundry-sdk)

- [Exercise: Create a generative AI chat app](https://microsoftlearning.github.io/mslearn-ai-studio/Instructions/02a-AI-foundry-sdk.html)


- [Develop a RAG-based solution with your own data using Azure AI Foundry](https://aka.ms/mslearn-ai-foundry-rag)

- [Exercise: Create a generative AI app that uses your own data](https://microsoftlearning.github.io/mslearn-ai-studio/Instructions/04-Use-own-data.html)

- [Fine-tune a language model with Azure AI Foundry](https://aka.ms/mslearn-ai-fine-tune)

- [Implement a responsible generative AI solution in Azure AI Foundry](https://aka.ms/mslearn-responsible-ai)


- [Exercise: Apply content filters to prevent the output of harmful content](https://microsoftlearning.github.io/mslearn-ai-studio/Instructions/06-Explore-content-filters.html)


- [Evaluate generative AI performance in Azure AI Foundry portal](https://aka.ms/mslearn-ai-evaluation)


<br>

## Learning Path 2: Develop AI agents on Azure

- [Get started with AI agent development on Azure](https://aka.ms/mslearn-get-started-agents)

- [Exercise: Explore AI Agent development](https://microsoftlearning.github.io/mslearn-ai-agents/Instructions/01-agent-fundamentals.html)


- [Develop an AI agent with Azure AI Foundry Agent Service](https://aka.ms/mslearn-develop-ai-agent)

- [Exercise: Develop an AI agent](https://microsoftlearning.github.io/mslearn-ai-agents/Instructions/02-build-ai-agent.html)


- [Integrate custom tools into your agent](https://aka.ms/mslearn-ai-agent-tools)


- [Develop a multi-agent solution with Azure AI Foundry Agent Service](https://aka.ms/mslearn-multi-agent)

- [Exercise: Develop a multi-agent solution](https://microsoftlearning.github.io/mslearn-ai-agents/Instructions/03b-build-multi-agent-solution.html)

- [Integrate MCP Tools with Azure AI Agents](https://aka.ms/mslearn-mcp)


- [Exercise: Connect AI agents to tools using Model Context Protocol (MCP)](https://microsoftlearning.github.io/mslearn-ai-agents/Instructions/03c-use-agent-tools-with-mcp.html)


- [Develop AI agents with the Semantic Kernel SDK](https://aka.ms/mslearn-ai-semantic-kernel-agent)

- [Develop AI agents with the Semantic Kernel multi agent](https://aka.ms/mslearn-ai-semantic-kernel-multi-agent)


- [Exercise: Develop an Azure AI agent with the Semantic Kernel SDK](https://microsoftlearning.github.io/mslearn-ai-agents/Instructions/04-semantic-kernel.html)

<br>

## Learning Path 3: Develop natural language solutions in Azure


- [Analyze text](https://aka.ms/mslearn-analyze-text)

- [Exercise: Analyze Text](https://microsoftlearning.github.io/mslearn-ai-language/Instructions/Labs/01-analyze-text.html)

- [Conversational understanding](https://aka.ms/mslearn-conversational-understanding)

- [Conversational question answering](https://aka.ms/mslearn-question-answering)


- [Exercise: Create a Question Answering Solution](https://microsoftlearning.github.io/mslearn-ai-language/Instructions/Labs/02-qna.html)

- [Custom classification and named entity extraction](https://aka.ms/mslearn-custom-text)


- [Exercise: Custom text classification](https://microsoftlearning.github.io/mslearn-ai-language/Instructions/Labs/04-text-classification.html)


- [Speech recognition and synthesis](https://aka.ms/mslearn-speech-app)


- [Exercise: Recognize and synthesize speech](https://microsoftlearning.github.io/mslearn-ai-language/Instructions/Labs/07-speech.html)


- [Translation text](https://aka.ms/mslearn-translate-text)

- [Translation speech](https://aka.ms/mslearn-translate-speech)

- [Audio enabled generative AI app](https://aka.ms/mslearn-generative-ai-audio) 


<br>

## Learning Path 4: Develop computer vision solutions in Azure


- [Analyze images](https://aka.ms/mslearn-analyze-images)

- [Exercise: Analyze images](https://microsoftlearning.github.io/mslearn-ai-vision/Instructions/Labs/01-analyze-images.html)

- [Read text in images](https://aka.ms/mslearn-read-text-images)

- [Exercise: Read text in images](https://microsoftlearning.github.io/mslearn-ai-vision/Instructions/Labs/02-ocr.html)


- [Detect, analyze, and recognize faces](https://aka.ms/mslearn-faces)

- [Exercise: Detect and analyze faces](https://microsoftlearning.github.io/mslearn-ai-vision/Instructions/Labs/03-face-service.html)

- [Classify images](https://aka.ms/mslearn-custom-image-classification)

- [Detect objects](https://aka.ms/mslearn-custom-object-detection)

- [Exercise: Classify images](https://microsoftlearning.github.io/mslearn-ai-vision/Instructions/Labs/04-image-classification.html)

- [Develop a vision-enabled generative AI application](https://aka.ms/mslearn-gen-ai-vision)

- [Exercise: Develop a vision-enabled chat app](https://microsoftlearning.github.io/mslearn-ai-vision/Instructions/Labs/08-gen-ai-vision.html)

- [Generate images with AI](https://aka.ms/mslearn-generate-images)

- [Exercise: Generate images with AI](https://microsoftlearning.github.io/mslearn-ai-vision/Instructions/Labs/09-dall-e.html)

<br>

## Learning Path 5: Develop AI information extraction solutions in Azure

- [Create a multimodal analysis solution with Azure AI Content Understanding](https://aka.ms/mslearn-multimodal-content)

- [Exercise: Extract information from multimodal content](https://microsoftlearning.github.io/mslearn-ai-information-extraction/Instructions/Labs/01-content-understanding.html)

- [Create an Azure AI Content Understanding client application](https://aka.ms/mslearn-ai-content-understanding-api)

- [Exercise: Develop a Content Understanding client application](https://microsoftlearning.github.io/mslearn-ai-information-extraction/Instructions/Labs/02-content-understanding-api.html)

- [Extract form data with Azure Document Intelligence prebuilt models](https://aka.ms/mslearn-doc-intelligence)

- [Exercise: Analyze forms with prebuilt Azure AI Document Intelligence models](https://microsoftlearning.github.io/mslearn-ai-information-extraction/Instructions/Labs/03-prebuilt-doc-intelligence-model.html)

- [Extract form data with Azure Document Intelligence custom models](https://aka.ms/mslearn-custom-doc-intelligence)

- [Exercise: Analyze forms with custom Azure AI Document Intelligence models](https://microsoftlearning.github.io/mslearn-ai-information-extraction/Instructions/Labs/04-custom-doc-intelligence-model.html)

- [Create a knowledge mining solution with Azure AI Search](https://aka.ms/mslearn-ai-knowledge-mining)


- [Exercise: Create a knowledge mining solution](https://microsoftlearning.github.io/mslearn-ai-information-extraction/Instructions/Labs/05-knowledge-mining.html)

<br>

